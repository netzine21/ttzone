(() => {
  ...
  function renderPublicLeagueStandings(game, format) {
    ...
    return `<div class="schedule-table-wrap"><table class="public-league-summary"><tbody>${groups.map((group) => { const standingGroup = standingGroups.get(group.name); const complete = Boolean(standingGroup?.complete); const participantCells = group.players.map((unit) => { const record = standingGroup?.standings.find((item) => item.player.playerKey === unit.playerKey); const rank = complete ? record?.rank : null; const members = unit.members?.length ? unit.members : [unit]; const names = members.map((member) => `${escapeHtml(member.nickname || unit.nickname || unit.label)} (${escapeHtml(member.rank || unit.rank || '-')})`).join(', '); const team = format === 'singles' ? unit.teamName || unit.members?.[0]?.teamName || '-' : unit.label || unit.teamName || '-'; return `<td class="public-league-participant"><div class="public-league-entry rank-row rank-row--${rank || 'pending'}"><div><strong>${names}</strong><small>${escapeHtml(team)}</small></div>${rank ? `<span class="rank-badge rank-badge--${rank}">${rank}위</span>` : ''}</div></td>`; }).join(''); const emptyCells = Array.from({ length: maxPlayers - group.players.length }, () => '<td class="public-league-participant public-league-participant--empty">-</td>').join(''); return `<tr><th class="public-league-summary__group">${escapeHtml(group.name)}</th>${participantCells}${emptyCells}</tr>`; }).join('')}</tbody></table></div>`;
  }
  ...
  function renderScheduleOperationPanel(games, game, formats, format, mode = 'generate') {
    ...
    return `
      <div class="schedule-panel">
        ...
        <div class="operation-note">${isResultsMode ? '생성된 예선 대진표의 경기 결과를 입력하고 저장합니다. 점수는 11점 5전 3선승 기준입니다.' : '조별 단일리그 대진표를 생성합니다. 같은 조의 선수 또는 팀이 서로 한 번씩 경기합니다.'}</div>
