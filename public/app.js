(() => {
  ...
  function renderScheduleResultsTable(matches, editable = true, format = 'singles') {
    ...
    return `<div class="schedule-table-wrap"><table class="schedule-table"><thead><tr><th>순번</th><th>라운드</th><th>조 / 탁구대</th><th>대진</th>${ruleColumn}<th>세트 스코어</th><th>승자</th></tr></thead><tbody>${matches.map((match, index) => `<tr><td>${index + 1}</td><td>${match.round}라운드</td><td>${escapeHtml(match.table)}</td><td><strong>${escapeHtml(match.sideA)}</strong><small>vs</small><strong>${escapeHtml(match.sideB)}</strong></td>${format === 'team' ? '' : '<td>11점 · 5전 3선승</td>'}<td>${editable ? `<input class="schedule-result-input" data-match-score="${escapeHtml(match.id)}" value="${escapeHtml(match.result?.score || '')}" placeholder="세트 스코어" />` : escapeHtml(match.result?.score || '-')}</td><td>${editable ? `<select data-match-winner="${escapeHtml(match.id)}"><option value="">미입력</option><option value="A" ${match.result?.winner === 'A' ? 'selected' : ''}>${escapeHtml(match.sideA)}</option><option value="B" ${match.result?.winner === 'B' ? 'selected' : ''}>${escapeHtml(match.sideB)}</option></select>` : escapeHtml(match.result?.winner === 'A' ? match.sideA : match.result?.winner === 'B' ? match.sideB : '-')}</td></tr>`).join('')}</tbody></table></div>`;
  }
    ...
    return `
      <div class="schedule-panel">
        <div class="operation-controls schedule-generation-controls">
          ...
          ${saved && !isResultsMode ? '<button type="button" class="game-list-action" data-print-schedule><svg class="game-list-action__icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M6 9V4h12v5M6 17H4V10h16v7h-2M6 14h12v6H6z"></path><path d="M17 12h1"></path></svg><span>대진표 인쇄</span></button>' : ''}
        </div>
  ...
  function renderOperationsPage(currentUser, embedded = false) {
    ...
    return `
      <section class="panel section-card operations-page${embedded ? ' operations-page--embedded' : ''}">
        ...
        ${state.operationMenu === 'print' ? `<div class="operation-submenu"><button type="button" class="operation-submenu__item ${state.operationSubmenu === 'qualifying' ? 'is-active' : ''}" data-operation-submenu="qualifying">리그전 대진표</button><button type="button" class="operation-submenu__item ${state.operationSubmenu === 'tournament' ? 'is-active' : ''}" data-operation-submenu="tournament">토너먼트 대진표</button></div>` : ''}
        ...
        ${state.operationMenu === 'roster' ? renderRosterOperationPanel(games, game, formats, format) : state.operationMenu === 'print' && state.operationSubmenu === 'qualifying' ? renderScheduleOperationPanel(games, game, formats, format, 'generate') : state.operationMenu === 'results' && state.operationSubmenu === 'qualifying' ? renderScheduleOperationPanel(games, game, formats, format, 'results') : state.operationMenu === 'print' && state.operationSubmenu === 'tournament' ? renderTournamentPrintPanel(games, game, formats, format) : state.operationMenu === 'results' && state.operationSubmenu === 'tournament' ? renderTournamentOperationPanel(games, game, formats, format) : `<div class="operation-controls qualifying-controls">
