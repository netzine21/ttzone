(() => {
  ...
  function renderPublicLeagueStandings(game, format) {
    ...
    const summary = `<div class="schedule-table-wrap"><table class="public-league-summary"><tbody>${groups.map((group) => { const standingGroup = standingGroups.get(group.name); const complete = Boolean(standingGroup?.complete); const participantCells = group.players.map((unit) => { const record = standingGroup?.standings.find((item) => item.player.playerKey === unit.playerKey); const rank = complete ? record?.rank : null; const members = unit.members?.length ? unit.members : [unit]; const names = members.map((member) => `<span class="public-league-entry__name">${escapeHtml(member.nickname || unit.nickname || unit.label)}</span><span class="public-league-entry__skill"> (${escapeHtml(member.rank || unit.rank || '-')})</span>`).join(', '); const team = format === 'singles' ? unit.teamName || unit.members?.[0]?.teamName || '-' : unit.label || unit.teamName || '-'; const recordText = record && (record.wins || record.losses || record.setsFor || record.setsAgainst) ? `<small class="public-league-entry__record">${record.wins}승 ${record.losses}패 · 세트 ${record.setsFor}-${record.setsAgainst}</small>` : ''; return `<td class="public-league-participant"><div class="public-league-entry rank-row rank-row--${rank || 'pending'}"><div><strong>${names}</strong><small>${escapeHtml(team)}</small>${recordText}</div>${rank ? `<span class="rank-badge rank-badge--${rank}">${rank}위</span>` : ''}</div></td>`; }).join(''); const emptyCells = Array.from({ length: maxPlayers - group.players.length }, () => '<td class="public-league-participant public-league-participant--empty">-</td>').join(''); return `<tr><th class="public-league-summary__group">${escapeHtml(group.name)}</th>${participantCells}${emptyCells}</tr>`; }).join('')}</tbody></table></div>`;
