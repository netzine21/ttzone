(() => {
  ...
  function renderOperationsPage(currentUser, embedded = false) {
    ...
    return `
      <section class="panel section-card operations-page${embedded ? ' operations-page--embedded' : ''}">
        ...
        ${state.operationMenu === 'results' ? '<div class="operation-submenu"><button type="button" class="operation-submenu__item is-active" data-operation-submenu="qualifying">예선리그 경기결과 입력</button></div>' : ''}
        ${state.operationMenu === 'roster' ? renderRosterOperationPanel(games, game, formats, format) : state.operationMenu === 'print' && state.operationSubmenu === 'qualifying' ? renderScheduleOperationPanel(games, game, formats, format, 'generate') : state.operationMenu === 'results' && state.operationSubmenu === 'qualifying' ? renderScheduleOperationPanel(games, game, formats, format, 'results') : state.operationMenu === 'print' && state.operationSubmenu === 'tournament' ? renderTournamentPrintPanel(games, game, formats, format) : state.operationMenu === 'results' && state.operationSubmenu === 'tournament' ? renderTournamentPrintPanel(games, game, formats, format) : `<div class="operation-controls qualifying-controls">
  ...
  function handlePrintAllSchedules() {
    ...
    printRoot.innerHTML = groups.map((group) => {
      const groupMatches = matches.filter((match) => match.groupName === group.name);
      return `<section class="schedule-print-group schedule-group-block"><div class="group-result-heading"><h3>${escapeHtml(group.name)} 대진표</h3><span>${escapeHtml(String(groupMatches.length))}경기 · ${escapeHtml(String(group.players.length))}명/팀</span></div>${renderScheduleMatrix(group, groupMatches)}<h4 class="schedule-order-title">${escapeHtml(group.name)} 경기 진행순서</h4>${renderScheduleResultsTable(groupMatches, false, format)}</section>`;
    }).join('');
    printRoot.querySelectorAll('.muted').forEach((element) => { element.textContent = ''; });
    document.body.appendChild(printRoot);
    const cleanup = () => document.body.classList.remove('is-printing-schedule');
